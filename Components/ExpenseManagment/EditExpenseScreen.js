...
const EditExpenseScreen = ({ navigation, route }) => {
  ...
  const [existingImage, setExistingImage] = useState(expense.receipt_image || null);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Append required fields based on API
      ...
      if (result.status == 200) {
        ...
        Alert.alert(translate('error'), result.message || translate('failedToUpdateExpense'));
      }
    } catch (error) {
      console.error('Update expense error:', error);
      Alert.alert(translate('error'), translate('networkErrorUpdatingExpense'));
    } finally {
      setLoading(false);
    }
  };
  ...
  // Get selected staff
  const selectedStaff = staff.find(s => s.id == formData.staff_id) || expense.added_by;
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.section}>
          <Text style={[styles.sectionTitle, isRTL && styles.arabicText]}>
            {translate('statusAndApproval')}
          </Text>
          
          <View style={styles.inputGroup}>
            <Text style={[styles.label, isRTL && styles.arabicText]}>
              {translate('expenseStatus')}
            </Text>
        ...
        <View style={styles.section}>
          ...
          {(selectedImage || existingImage) ? (
            <View style={styles.imageContainer}>
              <Image 
                source={{ uri: selectedImage ? selectedImage.uri : existingImage }} 
              ...
              >
                <Ionicons name="camera" size={16} color="#6B7D3D" />
                <Text style={[styles.changeImageText, isRTL && styles.arabicText]}>
                  {translate('changeImage')}
                </Text>
              </TouchableOpacity>
            </View>
          ) : (
...
// Use same styles as AddExpenseScreen
